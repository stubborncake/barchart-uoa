...
async def main():
    ...
    # Or fetch everything (~3,000+ records across all stocks)
    # await client.fetch(max_pages=None)

    # Query
    print(client.top(10))               # Top 10 by Vol/OI ratio
...
|---|---|---|
| `client.top(n=10)` | `list[dict]` | Top N records sorted by Vol/OI (or `key='volume'` etc.) |
| `client.by_symbol('AAPL')` | `list[dict]` | Filter records for a specific ticker. |
...
| Method | Description |
|---|---|
| `client.to_csv(filepath=None)` | Export to CSV (auto-names with timestamp if no path given). |
| `client.to_json(filepath=None)` | Export to JSON. |
